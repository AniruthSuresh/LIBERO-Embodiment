...
# We need to import robosuite to define the controller and for transform utils
...
# --- 2. Load 6D (XYZ, RPY) Trajectory from Demonstration ---
demo_file_path = "/home/aniruth/Desktop/RRC/LIBERO/LIBERO-Datasets/libero_10/KITCHEN_SCENE3_turn_on_the_stove_and_put_the_moka_pot_on_it_demo.hdf5"
# The exact key you specified
...
with h5py.File(demo_file_path, "r") as f:
    ...
    # We still need the gripper action and the initial scene setup
...
# --- 3. Setup Env with JOINT_POSITION Controller for IK ---
...
env_args = {
    ...
    "robots": ["Panda"],
    "controller": "IK_POSE", # Pass the controller config
    "camera_heights": 256,
    "camera_widths": 256,
}
env = OffScreenRenderEnv(**env_args)

# Set the scene to the initial state from the demonstration
obs = env.reset()
env.sim.set_state_from_flattened(initial_state)
env.sim.forward()

# FIX 2: Get a valid observation AFTER setting the state to initialize our variables
# This requires an internal method from the environment wrapper.
...
# --- 4. Main Loop: Calculate IK and Command Joints Directly ---
print("[info] Moving robot by calculating joint angles with Inverse Kinematics...")

# FIX 2 (continued): Initialize last_valid_joints before the loop starts
...
for t in tqdm(range(num_steps), desc="Sending EE Poses"):
    ...
    # 2. Get the axis-angle vector (called 'rotvec' in scipy)
