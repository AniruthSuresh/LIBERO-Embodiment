...
# --- 2. Find and Load the Demonstration File for the "Full Scene" ---
relative_demo_path = task_suite.get_task_demonstration(task_id)

# # Build the full, absolute path to the demonstration file to avoid errors
...
# Load the actions and initial state from the file
...
# --- 3. Set up the Environment and Output Folder ---
env_args = {
...
# Create a unique directory to save the image
...
# --- 4. Render and Save the Default Initial Scene ---
print("[info] Capturing the default initial image...")

# Get the image from the initial observation dictionary
initial_image = initial_obs["agentview_image"]

# Construct the filename for the single initial image
filename = os.path.join(output_dir, "default_initial_scene.png")

# Save the single image file (correcting for color and orientation)
